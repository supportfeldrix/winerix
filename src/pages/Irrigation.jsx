...
function Irrigation() {
  ...
  const load = useCallback(async () => {
    ...
    if (recRes.error) {
      const friendly = friendlyIrrigationError(recRes.error);
  ...
  const handleView = (record) => navigate(`/irrigation/${record.id}`);

  const handleSubmit = async (values) => {
    setSaving(true);
    const isEdit = Boolean(editing);
    const { error: err } = isEdit
      ? await updateIrrigationRecord(editing.id, values)
      : await createIrrigationRecord(values);
    setSaving(false);
    ...
    setFormOpen(false);
    setEditing(null);
    setToast(isEdit ? 'Irrigation record updated.' : 'Irrigation record added.');
    await load();
  };
  ...
  const handleDelete = async () => {
    ...
    setDeleting(true);
    const { error: err } = await deleteIrrigationRecord(deleteTarget.id);
    setDeleting(false);
    ...
    setToast('Irrigation record deleted.');
    await load();
  };
  ...
  return (
    <PageContainer>
      ...
      >
        <Box>
          ...
          <Typography variant="body1" sx={{ color: 'text.secondary' }}>
            Manage watering activities across your vineyards and blocks.
          </Typography>
        </Box>
        <Button
          variant="contained"
      ...
      {loading ? (
        ...
        >
          <Typography variant="body1" sx={{ color: 'text.secondary' }}>
            No irrigation records match your filters.
          </Typography>
        </Paper>
...
function EmptyIrrigation({ onAdd }) {
  return (
    <Paper
      variant="outlined"
      sx={{
    ...
    >
      ...
      >
        <WaterDropOutlinedIcon sx={{ fontSize: '2rem' }} />
      </Box>
      ...
      >
        Add your first irrigation record to start tracking watering activities
