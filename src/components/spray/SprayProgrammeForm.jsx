...
/**
 * Add / Edit spray programme dialog form.
 *
 * Fields: Spray Programme Title (required), Vineyard (required select), Block
...
function SprayProgrammeForm({
    ...
    Promise.all([getVineyardOptions(), getBlockOptions()]).then(
      ([vRes, bRes]) => {
        ...
        if (vRes.error || bRes.error) {
          setOptionsError(friendlySprayProgrammeError(vRes.error || bRes.error));
          setVineyards([]);
          setBlocks([]);
        } else {
  ...
  const validate = () => {
    ...
    if (!values.title.trim()) {
      next.title = 'Spray programme title is required.';
  ...
  return (
    ...
    >
      <DialogTitle>
        {isEdit ? 'Edit Spray Programme' : 'Add Spray Programme'}
      </DialogTitle>
      ...
      <DialogContent>
        <Stack spacing={2.5} sx={{ mt: 1 }}>
          ...
          {noVineyards && (
            <Alert severity="info">
              You need at least one vineyard before adding a spray programme.
              Create a vineyard first.
            </Alert>
          )}

          <TextField
            label="Spray Programme Title"
            value={values.title}
...
export default SprayProgrammeForm;
