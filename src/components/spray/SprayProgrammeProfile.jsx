...
import SprayProgrammeForm from './SprayProgrammeForm';
...
/**
 * Spray programme detail / profile page (route: /spray-programme/:id).
 * Loads a single spray programme with its vineyard and (optional) block,
 * supports edit and delete, and links to the parent vineyard / block.
 */
function SprayProgrammeProfile() {
  const { id } = useParams();
  ...
  const load = useCallback(async () => {
    setLoading(true);
    setError('');
    const { data, error: err } = await getSprayProgramme(id);
    if (err) {
  ...
  const handleEditSubmit = async (values) => {
    setSaving(true);
    const { data, error: err } = await updateSprayProgramme(id, values);
    setSaving(false);
    if (err) {
      setError(friendlySprayProgrammeError(err));
      return;
    ...
    setRecord(data);
    setEditOpen(false);
    setToast('Spray programme updated.');
  };

  const handleDelete = async () => {
    setDeleting(true);
    const { error: err } = await deleteSprayProgramme(id);
    setDeleting(false);
    ...
    navigate('/spray-programme', { replace: true });
  };
  ...
  return (
    <PageContainer>
      ...
      <Link
        ...
        type="button"
        underline="hover"
        onClick={() => navigate('/spray-programme')}
        sx={{
          display: 'inline-flex',
          alignItems: 'center',
          gap: 0.5,
...
export default SprayProgrammeProfile;
